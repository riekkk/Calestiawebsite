...
(function () {
  ...
  /* ------------------------------------------------------------------
     CONFIG
     ------------------------------------------------------------------ */
  // Same EmailJS account used by config.js's CALESTIA_EMAILJS_* constants
  // (admin-portal.js reuses it for the employee-invitation email) — the
  // literals here are just a fallback so this file still works standalone.
  var EMAILJS_PUBLIC_KEY = window.CALESTIA_EMAILJS_PUBLIC_KEY || 'Bp_1nflmnjqr7recy';
  var EMAILJS_SERVICE_ID = window.CALESTIA_EMAILJS_SERVICE_ID || 'service_y33oto9';
  ...
  function handleSignIn() {
    ...
    supabaseClient.auth.signInWithPassword({ email: email, password: password })
      .then(function (result) {
        ...
        showToast('Signed in! Redirecting to your portal...');
        closeAuthModal();
        var userId = result.data.user.id;
        setTimeout(function () { redirectToPortalForRole(userId); }, 700);
      })
      .catch(function () {
        btn.textContent = originalText;
        btn.disabled = false;
  ...
  function updateAuthUI(session) {
    ...
    applyRolePortalLinks(session.user.id, portalLinks);
  ...
  // Points nav "My Account" links (defaults to client-portal.html in the
  // HTML) at whichever portal this signed-in account's role actually
  // belongs to. Each portal's own gate independently re-verifies this —
  // this is just so the link doesn't send staff on a detour first.
  function applyRolePortalLinks(userId, portalLinks) {
    if (!portalLinks.length || !supabaseClient) return;
    supabaseClient.from('profiles').select('role').eq('id', userId).maybeSingle()
      .then(function (result) {
        var role = result.data && result.data.role;
        var url = (window.CALESTIA_ROLE_PORTAL_URLS || {})[role] || 'client-portal.html';
        for (var i = 0; i < portalLinks.length; i++) portalLinks[i].setAttribute('href', url);
      })
      .catch(function () {});
  }

  // After sign-in / password reset, land the account on the portal that
  // matches its real role (role is server-determined — profiles.role —
  // never chosen by the user). Falls back to the client portal if the
  // lookup fails; that portal's own gate will bounce staff onward.
  function redirectToPortalForRole(userId) {
    if (!supabaseClient) { window.location.href = 'client-portal.html'; return; }
    supabaseClient.from('profiles').select('role').eq('id', userId).maybeSingle()
      .then(function (result) {
        var role = result.data && result.data.role;
        window.location.href = (window.CALESTIA_ROLE_PORTAL_URLS || {})[role] || 'client-portal.html';
      })
      .catch(function () { window.location.href = 'client-portal.html'; });
  }
    ...
    form.addEventListener('submit', function (e) {
      ...
      supabaseClient.auth.updateUser({ password: pw })
        .then(function (result) {
          ...
          showToast('Password updated! Redirecting to your portal...');
          var userId = result.data && result.data.user && result.data.user.id;
          setTimeout(function () {
            if (userId) redirectToPortalForRole(userId);
